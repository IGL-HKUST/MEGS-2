...
__device__ float3 computeCov2D_fused(const glm::vec3 scale, float mod, const glm::vec4 rot, const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float* viewmatrix)
{ 
    float3 t = transformPoint4x3(mean, viewmatrix);
    ...
    glm::mat2x3 T = S * R * W * J;
    glm::mat2 cov = glm::transpose(T) * T;
	...
	bool prefiltered)
{
    ...
    float3 cov = computeCov2D_fused(scales[idx], scale_modifier, rotations[idx], p_orig, focal_x, focal_y, tan_fovx, tan_fovy, viewmatrix);
    ...
    // float my_radius = ceil(sqrt(1 * log(opacities[idx] * 255) * max(lambda1, lambda2)));
