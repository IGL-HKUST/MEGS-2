...
// std::function<torch::Tensor& (size_t N)> resizeTensorFunctional(torch::Tensor& t) { 
//     auto lambda = [&t](size_t N) {
//         t.resize_({(long long)N});
//     return t;
//     };
...
//   torch::Tensor accum_max_count = torch::full({P}, 0, float_opts);
  ...
  torch::Tensor geomTempBuffer = torch::empty({0}, options.device(device));
  torch::Tensor binningBuffer = torch::empty({0}, options.device(device));
  torch::Tensor imgBuffer = torch::empty({0}, options.device(device));
  std::function<char*(size_t)> geomFunc = resizeFunctional(geomBuffer);
  std::function<char*(size_t)> geomTempFunc = resizeFunctional(geomTempBuffer);
...
//   const torch::Tensor& geomTempBuffer,
//   const int R,
...
//     reinterpret_cast<char*>(geomTempBuffer.contiguous().data_ptr()),
